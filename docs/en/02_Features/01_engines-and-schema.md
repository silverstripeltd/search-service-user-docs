...
You can view your engine's current schema using the [Silverstripe Search Administration module](https://github.com/silverstripeltd/silverstripe-bifrost-admin) or with the API. Schema must be created or updated by an API request. For more information, refer to the [Developer’s Guide](/developers-guide.md).
